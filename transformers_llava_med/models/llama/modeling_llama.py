...
from ...modeling_outputs import BaseModelOutputWithPast, BaseMoEModelOutputWithPast, CausalLMOutputWithPast, SequenceClassifierOutputWithPast
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights_drop = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
...
def my_scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None, return_attention_weights=False) -> torch.Tensor:
    ...
    if attn_mask is not None:
        # print(attn_mask.shape, attn_bias.shape, "attention mask shape")
        if attn_mask.dtype == torch.bool:
            attn_bias.masked_fill_(attn_mask.logical_not(), float("-inf"))
        else:
            attn_bias += attn_mask.squeeze(0).squeeze(0)
            # attn_bias += attn_mask.unsqueeze(0).unsqueeze(0).expand_as(attn_bias)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.use_moe:
            return attn_output, attn_weights, past_key_value, k_moe_routing
        return attn_output, attn_weights, past_key_value
