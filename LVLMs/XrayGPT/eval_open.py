...
def eval_model(args):
    ...
    # answers_file = os.path.expanduser(args.answers_file)
    # answers_file_all = "/data/aofei/hallucination/mimic_cxr/open_inference/XrayGPT/mimic_open1_res.jsonl"
    # if answers_file_all.endswith('.jsonl'):
    #     with open(answers_file_all, 'r') as file:
    #         answers = [json.loads(line) for line in file]
    # else:
    #     with open(answers_file_all, 'r') as file:
    #         answers = json.load(file)
    # existing_ids = set([i["question_id"] for i in answers])
    need_questions = []
    for i in questions:
