...
def show_image_relevance(image_relevance, image, orig_image, preprocess, mask=None, only_map=False, show_mask=False, att_hw=(24,24)):
    ...
    if only_map:
        ...
        im = plt.imshow(image_relevance, cmap='jet')  # Plot the heatmap for colorbar
        plt.axis('off')
        plt.colorbar(im, orientation='vertical', label='Relevance')  # Add the colorbar
        plt.imshow(vis, alpha=0.6)  # Overlay the visualized image on top
        
        # plt.imshow(vis)
        # # plt.imshow(image)
        # plt.axis('off')
        # plt.colorbar(plt.cm.ScalarMappable(cmap='jet'), orientation='vertical', label='Attention score')  # Add colorbar
        if show_mask:
        ...
        fig.colorbar(plt.cm.ScalarMappable(cmap='jet'), ax=axs[1], orientation='vertical', label='Relevance')  # Add colorbar
