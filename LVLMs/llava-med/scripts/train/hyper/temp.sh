...
for seed in "${seeds[@]}"; do
    ...
    python llava/eval/eval_batch.py --num-chunks 4  --model-name ${ROOT_PATH}/LLM/llava_med \
        --question-file ${ROOT_PATH}/hallucination/${dataset}/data/with_gt/test_with_gt.json \
        --image-folder ${ROOT_PATH}/hallucination/${dataset}/imgs \
        --answers-file ${ROOT_PATH}/hallucination/mitigation/${dataset}/${dir}/inference/pred_ori.jsonl 

    python llava/eval/run_eval.py \
        --gt ${ROOT_PATH}/hallucination/${dataset}/data/with_gt/test_with_gt.json \
        --pred ${ROOT_PATH}/hallucination/mitigation/${dataset}/${dir}/inference/pred_ori.jsonl \
        --eval_res ${ROOT_PATH}/hallucination/mitigation/${dataset}/${dir}/inference/eval_res_ori.txt
...
# visual_enhance_ratio=0
bbox_ratio=0.25
bbox_ratios=(0.1 0.2 0.25 0.3)
epoch_num=9

for bbox_ratio in "${bbox_ratios[@]}"; do
    echo "Running with seed: $seed, bbox_ratio: $bbox_ratio"
    dir=llava_med/gt_exp/only_gt/all_expert_4_8_rank16/lora_bbox_${bbox_ratio}/epoch${epoch_num}_seed${seed}
    ...
    torchrun --nnodes=1 --nproc_per_node=4 --master_port=25001 \
        ...
        --mm_projector_lr 2e-5 \
        --tune_mm_mlp_adapter False \
        --data_path ${ROOT_PATH}/hallucination/${dataset}/data/with_gt/training_only_wgt.json \
        --image_folder ${ROOT_PATH}/hallucination/${dataset}/imgs \
        --vision_tower openai/clip-vit-large-patch14 \
        --mm_vision_select_layer -2 \
        --mm_use_im_start_end True \
...
for bbox_ratio in "${bbox_ratios[@]}"; do
    echo "Running with seed: $seed, bbox_ratio: $bbox_ratio"
    # dir=llava_med/gt_exp/all_expert_4_8_rank16/lora_${visual_enhance_ratio}_bbox_${bbox_ratio}/epoch${epoch_num}_seed${seed}
    dir=llava_med/gt_exp/weak_bbox/all_expert_4_8_rank16/lora_bbox_${bbox_ratio}/epoch${epoch_num}_seed${seed}
...
# visual_enhance_ratio=0
bbox_ratio=0.03
# bbox_ratios=(0.01 0.03 0.05 0.1)
bbox_ratios=(0.15 0.2 0.3)
epoch_num=6
head_nums=(64 128 256 512 1024)
# for seed in "${seeds[@]}"; do
for bbox_r in "${bbox_ratios[@]}"; do
    # echo "Running with seed: $seed"
    echo "Running with bbox ratio: $bbox_r"
    dir=llava_med/ours_v1/top128_balance/all_expert_8_16_rank16/lora_${visual_enhance_ratio}_bbox_${bbox_r}/epoch${epoch_num}_seed${seed}
...
head_nums=(64 256 512 1024)
# for seed in "${seeds[@]}"; do
for head_num in "${head_nums[@]}"; do
    # echo "Running with seed: $seed"
    echo "Running with bbox ratio: $bbox_r, head num: $head_num"
    dir=llava_med/ours_v1/top128_balance/head_${head_num}/lora_${visual_enhance_ratio}_bbox_${bbox_r}/epoch${epoch_num}_seed${seed}
