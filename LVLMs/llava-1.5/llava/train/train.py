...
import sys
sys.path.append("/home/avc6555/research/MedH/Mitigation/LVLMs/llava-1.5")
import torch
...
from llava.model.moe_llava import LoRA_MOE_QK
...
# def preprocess_bbox_masks(image_path, bboxes, processor):
#     iw, ih = image.size
#     return None
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            image_for_mask = Image.open(os.path.join(image_folder, image_file))
            image = image_for_mask.convert('RGB')
        ...
        # print(data_dict)
        # image_start_index = torch.where(data_dict['input_ids'] == -200)
        # print(image_start_index, "iamge token")
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Assume pad_token_id
        # pad_token_id = self.tokenizer.pad_token_id  # Set this to the actual pad_token_id of your tokenizer

        # # 1. Create a causal mask to prevent looking ahead
        # # This creates a lower triangular matrix, which prevents future tokens from being seen
        # causal_mask = torch.tril(torch.ones((seq_len, seq_len), dtype=torch.bool))
        # # Shape: (seq_len, seq_len)

        # # 2. Create a padding mask
        # # Padding mask where `1` means a valid token, and `0` means a padding token
        # padding_mask = input_ids.ne(pad_token_id).unsqueeze(1)  # Shape: (batch_size, 1, seq_len)

        # # 3. Combine the causal and padding mask
        # # Broadcast the padding mask to be compatible with the causal mask, and combine them
        # attention_mask = causal_mask.unsqueeze(0) & padding_mask  # Shape: (batch_size, seq_len, seq_len)

        # # 4. Convert boolean mask to integer (0/1)
