# then search hyper-parameters using rad
seeds=(4)
seed=4
...
dataset=VQA_RAD
# bbox_ratio=0.06
# bbox_ratios=(0.03 0.09 0.12 0.15)
bbox_ratios=(0.12)
...
for bbox_ratio in "${bbox_ratios[@]}"; do
    ...
    echo "Running with bbox_ratio: $bbox_ratio"
    dir=llava_med_1.5/ours_v2_true/balance_0.01_expert_${q_expert_num}_${k_expert_num}_rank16_8_lora_${visual_enhance_ratio}_bbox_${bbox_ratio}/head${head}/epoch${epoch_num}_seed${seed}
    ...
    torchrun --nnodes=1 --nproc_per_node=4 --master_port=25001 llava/train/train_mem.py \
        ...
        --segment_path ${ROOT_PATH}/hallucination/${dataset}/data/training_segments_top8.npz \
        ...
        --dense_moe True \
        --use_mask True \
        --query_expert_num $q_expert_num \
        --visual_expert_num $k_expert_num \
        --seed $seed \
        --top_heads $head \
        --use_kl True \
        --moe_balance_ratio 0.01 \
        --top_visual_moe_experts $top_k_num
        # --report_to wandb \
