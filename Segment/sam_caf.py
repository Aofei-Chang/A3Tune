...
def hyper_params_tuning(sam):
    ...
    best_params = {
                'points_per_side': points_per_side_range[1],
                'pred_iou_thresh': pred_iou_thresh_range[1],
                'stability_score_thresh': stability_score_thresh_range[1],
                "box_nms_thresh": box_nms_thresh_range[1], 
                "crop_nms_thresh" : crop_nms_thresh_range[1],
                "area": area_list[1]
            }
...
def retrieve_relevant_crop(crops, class_names, model, preprocess, config):
    ...
    # preprocessed_images = [preprocess(image) for image in pil_images]
...
def retrieve_relevant_crop_biomed_topk(crops, class_names, model, preprocess, config, tokenizer, topk=2):
    # Convert crops to uint8 images
    crops_uint8 = [image.astype(np.uint8) for image in crops]

    # Convert uint8 images to PIL images
    pil_images = []
    for image in crops_uint8:
        if image.shape[0] > 0 and image.shape[1] > 0:
            pil_image = Image.fromarray(image)
    ...
    # Preprocess images and move to CUDA
    preprocessed_images = [preprocess(image).to("cuda") for image in pil_images]
    if len(preprocessed_images) == 0:
        return None, None
    stacked_images = torch.stack(preprocessed_images)

    # Initialize similarity score dictionary
    similarity_scores = {class_name: [] for class_name in class_names}
    ...
    with torch.no_grad():
        ...
        for class_name in class_names:
            class_descriptions = class_names[class_name]
            class_text_features = [model.encode_text(tokenizer(description).to("cuda")) for description in class_descriptions]

            # Compute mean text features and normalize
            mean_text_feature = torch.mean(torch.stack(class_text_features), dim=0)
            mean_text_feature /= mean_text_feature.norm(dim=-1, keepdim=True)
            
            # Compute similarity scores between image and class text features
            similarity_score = 100. * image_features @ mean_text_feature.T
            similarity_scores[class_name] = similarity_score.squeeze().tolist()
            if type(similarity_scores[class_name]) is float:
                similarity_scores[class_name] = [similarity_scores[class_name]]

    # Get the top-k indices for each class
    topk_indices = {key: sorted(range(len(similarity_scores[key])), key=lambda i: similarity_scores[key][i], reverse=True)[:topk] for key in similarity_scores}

    return topk_indices, similarity_scores


def retrieve_relevant_crop_biomed(crops, class_names, model, preprocess, config, tokenizer):
    crops_uint8 = [image.astype(np.uint8) for image in crops]
